$(function() {
	...
	hall.bind('loaded', function() {
		...
		if ($.url.get('auto_watch') === 'true') {
			return $(this).oneTime('2s',watchGameHandler);
		}
	});
	...
	watchGameHandler =  function() {
		var gid = game_list.find('.selected').data('gid');
	...
	$.pp.reg("GAME_INFO", function(game_info) {
		game_counter++;
		$(template).data('gid', game_info.id).children('.gid').text(game_info.id).parent().children('.name').text(game_info.name).parent().children('.blind').text(game_info.low + " / " + game_info.high).parent().children(
				'.player').text(game_info.joined + " / " + game_info.seats).parent().children('.limit').text(game_info.min + " / " + game_info.max).parent()
				.insertBefore('.autofill').hover(function() {
							var selected = 'selected';
							if ($(this).hasClass(selected))
								return;
							$(this).parent().children().removeClass(selected);
							$(this).addClass(selected);
						}).dblclick(watchGameHandler);
		if (game_counter === game_info.count)
			hall.trigger('loaded');
	});
